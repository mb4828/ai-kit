...
5. Run `bash scripts/link-skill.sh <name>` from this package. It adds the appropriate Claude adapter for either direct use or an `.agents` mount and, when the local `mattbrauner` vault shortcut exists, the vault's Claude adapter. Never copy a skill into another `.agents` directory.
